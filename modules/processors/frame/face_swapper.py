...
def _face_mask_paste_back(target_img: Frame, bgr_fake: np.ndarray, M: np.ndarray, face_mask: np.ndarray) -> Frame:
    ...
    bgr_fake_crop = cv2.warpAffine(bgr_fake, IM_crop, (crop_w, crop_h), borderMode=cv2.BORDER_REPLICATE)
    mask_crop = face_mask[y1:y2, x1:x2]  # already float32 [0, 1]
    ...
    if getattr(modules.globals, "poisson_blend", False):
        ...
        if face_mask is not None:
            ...
            if len(x_indices) > 0 and len(y_indices) > 0:
                ...
                # seamlessClone requires uint8 mask [0, 255]
                mask_crop = (face_mask[y_min : y_max + 1, x_min : x_max + 1] * 255).astype(np.uint8)
...
def create_face_mask(face: Face, frame: Frame) -> "np.ndarray | None":
    """Returns a float32 [0,1] feathered convex-hull face mask in frame space.

    Delegates to build_face_hull_mask (shared, cached) so the swapper and
    enhancer both hit the same single-slot cache — computed once per frame.
    """
    if frame is None or not hasattr(frame, "shape") or len(frame.shape) < 2:
        return None
    return build_face_hull_mask(face, frame.shape)
